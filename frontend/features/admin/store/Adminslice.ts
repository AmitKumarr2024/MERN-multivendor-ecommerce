...
>("admin/fetchProducts", async (params, { rejectWithValue }) => {
  try {
    const { data } = await api.get<PaginatedAdminProducts>(
      `${BASE_URL}/products`,
      {
        params: params ?? undefined,
      },
...
export const fetchReviewsAdmin = createAsyncThunk<
  PaginatedAdminReviews,
  AdminReviewQueryParams | void,
  { rejectValue: string }
>("admin/fetchReviews", async (params, { rejectWithValue }) => {
  try {
    const { data } = await api.get<PaginatedAdminReviews>(
      `${BASE_URL}/reviews`,
      {
        params: params ?? undefined,
      },
...
export const forceDeleteReviewAdmin = createAsyncThunk<
  string,
  string,
  { rejectValue: string }
>("admin/deleteReview", async (id, { rejectWithValue }) => {
  try {
    await api.delete(`${BASE_URL}/reviews/${id}`);
    return id;
  } catch (error) {
    return rejectWithValue(getErrorMessage(error));
  }
...
interface AdminState {
  ...
  reviews: PaginatedAdminReviews["reviews"];
  reviewsTotal: number;
  reviewsPage: number;
  reviewsPages: number;
...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
      ...
      .addCase(updateUserRole.fulfilled, (state, action) => {
        ...
        if (user)
          user.role = action.payload
            .role as AdminState["users"][number]["role"];
    ...
    builder
      .addCase(forceDeleteReviewAdmin.pending, (state, action) => {
        state.mutatingId = action.meta.arg;
      })
      .addCase(forceDeleteReviewAdmin.fulfilled, (state, action) => {
        state.mutatingId = null;
        state.reviews = state.reviews.filter((r) => r._id !== action.payload);
        state.reviewsTotal = Math.max(0, state.reviewsTotal - 1);
        state.successMessage = "Review deleted";
      })
      .addCase(forceDeleteReviewAdmin.rejected, (state, action) => {
        state.mutatingId = null;
...
export const { clearAdminError, clearAdminMessage, clearLastResetPassword } =
  adminSlice.actions;
