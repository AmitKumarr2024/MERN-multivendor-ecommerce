...
export const fetchMyCart = createAsyncThunk<
  ...
  void,
  { rejectValue: string }
>("cart/fetchMine", async (_, { rejectWithValue }) => {
  try {
...
/* =========================================================
   ...
   ---------------------------------------------------------
   variantId goes in the body - this route IS validated by
   updateItemSchema (Zod), which now explicitly lists
   variantId, so it survives the validate() middleware.
========================================================= */
...
>(
  ...
  async ({ productId, quantity, variantId = null }, { rejectWithValue }) => {
    try {
      const { data } = await api.put<Cart>(`${BASE_URL}/items/${productId}`, {
        quantity,
        variantId,
      });
      return data;
...
/* =========================================================
   ...
   ---------------------------------------------------------
   This route has NO validate() middleware, so there's no Zod
   stripping to worry about - but axios still needs an explicit
   `params` config to attach a query string to a DELETE request
   (there's no body-shorthand like api.delete(url, payload)).
   The controller reads req.query.variantId first, so this is
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(removeCartItem.pending, (state, action) => {
        // action.meta.arg is now { productId, variantId } instead
        // of a bare string - pull productId out for the spinner.
        state.mutatingProductId = action.meta.arg.productId;
        state.error = null;
      })
