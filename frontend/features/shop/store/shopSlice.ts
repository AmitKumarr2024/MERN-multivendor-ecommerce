...
>("shop/fetchBySlug", async (slug, { rejectWithValue }) => {
  try {
    ...
    console.log("=================================");
    console.log("🏪 GET SHOP BY SLUG");
    console.log("=================================");
    console.log("Shop:", data);
    console.log("Address:", data.address);
    console.log("Address JSON:", JSON.stringify(data.address, null, 2));
    return data;
  } catch (error) {
    return rejectWithValue(getErrorMessage(error));
  }
...
>("shop/checkIsOpen", async (slug, { rejectWithValue }) => {
  try {
    const { data } = await api.get<IsOpenResponse>(
      `${BASE_URL}/${slug}/is-open`,
    );
    return data;
  } catch (error) {
    return rejectWithValue(getErrorMessage(error));
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updateBusinessHours.fulfilled, (state, action) => {
        state.mutating = false;
        if (state.myShop)
          state.myShop.businessHours = action.payload.businessHours;
    ...
    builder
      ...
      .addCase(updateHolidayDates.fulfilled, (state, action) => {
        if (state.myShop)
          state.myShop.holidayDates = action.payload.holidayDates;
