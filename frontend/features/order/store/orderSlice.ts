...
import type { TrackingResponse } from "@/features/logistics";
...
// Backend returns { shipment, liveTracking } (see logistics.controller.js's
// getTracking) — NOT just the shipment sub-document.
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(cancelMyOrder.fulfilled, (state, action) => {
        ...
        // 👇 ADD — currentOrder bhi update karo, warna OrderDetail page
        // (jo selectCurrentOrder use karta hai) stale rehta hai refresh tak
        if (
          state.currentOrder &&
          state.currentOrder._id === action.payload.id
        ) {
    ...
    builder
      ...
      .addCase(fetchOrderTracking.rejected, (state, action) => {
        state.trackingLoading = false;
        state.error = action.payload || "Failed to load tracking";
      });
    ...
    builder
      ...
      .addCase(shipOrder.fulfilled, (state, action) => {
        state.mutatingOrderId = null;
        const idx = state.shopOrders.findIndex(
...
export const { clearOrderError, clearOrderMessage, clearCurrentOrder } =
  orderSlice.actions;
